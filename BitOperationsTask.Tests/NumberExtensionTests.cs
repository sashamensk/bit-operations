...
namespace BitOperationsTask.Tests
{
    public class NumberExtensionTests
    {
        ...
        public int InsertNumberIntoAnotherTests(int sourceNumber, int anotherNumber, int i, int j)
        => InsertNumberIntoAnother(sourceNumber, anotherNumber, i, j);

        [Test]
        public void InsertNumberIntoAnotherTests_ThrowArgumentException_If_i_MoreThan_j() =>
            Assert.Throws<ArgumentException>(() => InsertNumberIntoAnother(8, 15, 8, 3),
                message: $"i should be less than or equal to j.");
        [Test]
        public void InsertNumberIntoAnother_ThrowArgumentOutOfRangeException_If_i_Equals_MinusOne() =>
